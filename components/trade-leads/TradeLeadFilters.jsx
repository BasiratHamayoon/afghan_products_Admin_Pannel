...
export default function TradeLeadFilter({
  query, onQueryChange,
  filters, onFilterChange,
  onResetFilters, hasActiveFilters,
  showFilters, onToggleFilters,
  sortBy, onSortChange, resultCount,
}) {
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex-1 min-w-[180px]">
          <SearchInput value={query} onChange={onQueryChange} placeholder="Search leads, categories, users..." />
        </div>
        ...
        {showFilters && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.22 }} className="overflow-hidden">
            <div className="flex items-center gap-2 flex-wrap pt-1">
              ...
              <FilterDropdown label="Priority" value={filters.priority || "all"} options={priorityOptions} onChange={(v) => onFilterChange("priority", v)} />
              {hasActiveFilters && (
                <button onClick={onResetFilters} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 cursor-pointer border border-red-200 dark:border-red-800/40 transition-colors">
