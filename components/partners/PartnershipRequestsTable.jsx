...
import {
  Eye, CheckCircle, XCircle, MapPin, DollarSign,
} from "lucide-react";
...
export default function PartnershipRequestsTable({
  items = [],
  onView,
  onApprove,
  onReject,
      ...
      <table className="w-full">
        ...
        <tbody>
          {safeItems.map((item, i) => {
            ...
            const approval =
              APPROVAL_CONFIG[item.approvalStatus] ||
            ...
            return (
              ...
              >
                ...
                <td className="py-4 px-4">
                  <span className="text-[11px] font-bold px-2.5 py-1 rounded-lg bg-blue-500/10 text-blue-600 capitalize">
                ...
                <td className="py-4 px-4">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-xs font-medium text-muted-foreground whitespace-nowrap">
                      {formatAmount(item.investmentRange?.min)} -{" "}
                      {formatAmount(item.investmentRange?.max)}
                    </span>
                  </div>
                </td>
                ...
                <td className="py-4 px-4">
                  <span className="text-xs font-medium text-muted-foreground whitespace-nowrap">
                    {item.equityOffered?.min ?? "—"}% -{" "}
                    {item.equityOffered?.max ?? "—"}%
                  </span>
                </td>
