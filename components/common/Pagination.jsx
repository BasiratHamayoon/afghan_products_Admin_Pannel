  ...
  return (
    <div className="flex items-center justify-between px-2 py-3">
      <p className="text-xs text-muted-foreground font-medium">
        ...
        <span className="font-bold text-foreground">{total}</span> {total !== 1 ? t("users.resultsPlural") : t("users.results")}
      </p>
      <div className="flex items-center gap-1">
        ...
        {pages.map((page) => (
          <button
            ...
            className={cn("h-8 w-8 rounded-lg text-xs font-bold transition-all cursor-pointer", page === currentPage ? "bg-[#0F69B0] text-white shadow-md shadow-[#0F69B0]/25" : "border border-gray-200 dark:border-white/[0.08] text-muted-foreground hover:bg-gray-50 dark:hover:bg-white/[0.04]")}
          >
            {page}
          </button>
        ))}
