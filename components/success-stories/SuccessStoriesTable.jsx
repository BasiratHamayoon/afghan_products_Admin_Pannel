      ...
      <table className="w-full">
        <thead>
          <tr style={{ borderBottom: "2px solid rgba(15,105,176,0.06)" }}>
            {["Person", "Company", "Rating", "Location", "Order", "Status", "Date", "Actions"].map((h) => (
          ...
          {safeItems.map((item, i) => {
            ...
            return (
              <motion.tr key={item.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="group border-b border-gray-50 dark:border-white/[0.03] last:border-0 hover:bg-gray-50/60 dark:hover:bg-white/[0.02] transition-colors">
                ...
                <td className="py-4 px-4">
                  <p className="text-xs font-medium text-muted-foreground truncate max-w-[120px]">{item.companyName || "—"}</p>
                </td>
                ...
                <td className="py-4 px-4">
                  <span className="text-xs font-medium text-muted-foreground">{formatDate(item.storyDate || item.createdAt)}</span>
                </td>
