...
import { clearSubCategoryOptions } from "@/store/slices/selectSlice";
import SearchableSelect from "@/components/common/SearchableSelect";

export default function ProductTypeForm({ initialData, onSubmit, onCancel, isLoading }) {
  const dispatch = useDispatch();
  const { categoryOptions, categoryOptionsLoading, subCategoryOptions, subCategoryOptionsLoading } = useSelector((state) => state.select);
  const safe = initialData && typeof initialData === "object" ? initialData : {};

  const [categoryId, setCategoryId] = useState(safe.categoryId || safe.category?._id || safe.category?.id || "");
  const [subCategoryId, setSubCategoryId] = useState(safe.subCategoryId || safe.subCategory?._id || safe.subCategory?.id || "");
  ...
  const categorySelectOptions = categoryOptions.map((c) => ({ value: c.id || c._id, label: c.name }));
  const subCategorySelectOptions = subCategoryOptions.map((s) => ({ value: s.id || s._id, label: s.name }));
  ...
  const handleSubmit = (e) => {
    ...
    if (!name.trim()) errs.name = "Product type name is required";
  ...
  return (
    <motion.form initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <SearchableSelect
              value={subCategoryId}
              onChange={(val) => { setSubCategoryId(val); if (errors.subCategoryId) setErrors((p) => ({ ...p, subCategoryId: "" })); }}
              options={subCategorySelectOptions}
              placeholder={!categoryId ? "Select a category first" : "Select a subcategory..."}
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              disabled={isLoading}
              className={cn("w-full px-4 py-3 rounded-xl text-sm font-medium outline-none transition-all border bg-white dark:bg-white/[0.04] text-foreground placeholder:text-muted-foreground/40 cursor-text disabled:opacity-60", errors.name ? "border-red-400" : "border-gray-200 dark:border-white/[0.08] focus:border-[#0F69B0]/40 focus:shadow-[0_0_0_3px_rgba(15,105,176,0.08)]")}
            />
          ...
          <div className="flex items-center gap-3 p-4 rounded-xl border border-gray-200 dark:border-white/[0.08] bg-gray-50 dark:bg-white/[0.02]">
            ...
            <div>
              <p className="text-sm font-bold text-foreground">Archive Product Type</p>
              <p className="text-[11px] text-muted-foreground font-medium">Archived product types are hidden from the store</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-5">
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-foreground uppercase tracking-widest">Product Type Image</label>
            <input ref={fileInputRef} type="file" accept="image/*" onChange={handleImageChange} className="hidden" id="producttype-image-upload" />
            {imagePreview ? (
              <div className="relative w-full h-48 rounded-xl overflow-hidden border border-gray-200 dark:border-white/[0.08] group">
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100">
                  ...
                  <button type="button" onClick={handleRemoveImage} disabled={isLoading} className="h-9 w-9 rounded-xl bg-red-500/90 flex items-center justify-center text-white hover:bg-red-500 transition-colors cursor-pointer"><X className="h-4 w-4" /></button>
                </div>
              </div>
            ) : (
              <label htmlFor="producttype-image-upload" className={cn("flex flex-col items-center justify-center w-full h-48 rounded-xl border-2 border-dashed transition-all cursor-pointer", isLoading ? "opacity-60 cursor-not-allowed border-gray-200 dark:border-white/[0.08]" : "border-gray-200 dark:border-white/[0.08] hover:border-[#0F69B0]/40 hover:bg-[#0F69B0]/[0.02]")}>
      ...
      <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 dark:border-white/[0.06]">
        ...
        <button type="submit" disabled={isLoading || categoryOptionsLoading} className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold text-white transition-all cursor-pointer shadow-lg shadow-[#0F69B0]/25 disabled:opacity-60 disabled:cursor-not-allowed" style={{ background: "linear-gradient(135deg, #0F69B0 0%, #0c5a9e 100%)" }}>
          {isLoading ? (<><Loader2 className="h-4 w-4 animate-spin" />Saving...</>) : (<><Save className="h-4 w-4" />{safe.id || safe._id ? "Update Product Type" : "Create Product Type"}</>)}
        </button>
      </div>
    </motion.form>
  );
