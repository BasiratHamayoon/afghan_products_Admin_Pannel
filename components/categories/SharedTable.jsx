      ...
      <table className="w-full">
        ...
        <tbody>
          {safeItems.map((item, i) => {
            ...
            return (
              <motion.tr key={item.id || item._id || i} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="group border-b border-gray-50 dark:border-white/[0.03] last:border-0 hover:bg-gray-50/60 dark:hover:bg-white/[0.02] transition-colors">
                <td className="py-4 px-4">
                  <div className="flex items-center gap-3">
                    <div className="h-10 w-10 rounded-xl overflow-hidden border border-gray-100 dark:border-white/[0.08] shrink-0 bg-gray-50 dark:bg-white/[0.04] flex items-center justify-center">
                      {imageUrl ? (
                        <img src={imageUrl} alt={item.name || ""} className="object-cover w-full h-full" onError={(e) => { e.target.style.display = "none"; if (e.target.nextSibling) e.target.nextSibling.style.display = "flex"; }} />
                      ) : null}
                      <span className="text-xl" style={{ display: imageUrl ? "none" : "flex" }}>📦</span>
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-foreground truncate max-w-[160px]">{item.name || "—"}</p>
                      <p className="text-[10px] text-muted-foreground font-medium mt-0.5">{item.slug || `ID: ${item.id || item._id}`}</p>
                    </div>
                  </div>
                </td>
                {safeColumns.map((col) => (
                  <td key={col.key} className="py-4 px-4">
                    <span className="text-xs font-medium text-muted-foreground">{item[col.key] ?? "—"}</span>
                  </td>
                ))}
