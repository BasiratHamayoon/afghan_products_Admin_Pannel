...
import { Eye, Edit2, Archive, ArchiveRestore } from "lucide-react";
import { formatDate } from "@/lib/helpers";
import { getFileUrl } from "@/lib/fileUrl";
import { cn } from "@/lib/utils";
import Image from "next/image";

export default function ProductTypeGridCard({
  item,
  index,
  onView,
  onEdit,
    ...
    >
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {item.categoryName && (
            <p className="text-[10px] text-[#0F69B0] font-semibold mt-0.5 truncate">
              {item.categoryName}
            </p>
          )}
          {item.subCategoryName && (
            <p className="text-[10px] text-purple-500 font-semibold truncate">
              ↳ {item.subCategoryName}
            </p>
          )}
        </div>
