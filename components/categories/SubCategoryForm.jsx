...
export default function SubCategoryForm({ initialData, onSubmit, onCancel, isLoading }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const errs = {};
    if (!name.trim()) errs.name = "Subcategory name is required";
    ...
    <motion.form initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-5">
          <div className="space-y-1.5">
            ...
            <SearchableSelect
              value={categoryId}
              onChange={(val) => { setCategoryId(val); if (errors.categoryId) setErrors((p) => ({ ...p, categoryId: "" })); }}
          ...
          <div className="flex items-center gap-3 p-4 rounded-xl border border-gray-200 dark:border-white/[0.08] bg-gray-50 dark:bg-white/[0.02]">
            ...
            <div>
              <p className="text-sm font-bold text-foreground">Archive Subcategory</p>
              <p className="text-[11px] text-muted-foreground font-medium">Archived subcategories are hidden from the store</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-5">
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-foreground uppercase tracking-widest">Subcategory Image</label>
            <input ref={fileInputRef} type="file" accept="image/*" onChange={handleImageChange} className="hidden" id="subcategory-image-upload" />
            {imagePreview ? (
              ...
              <label htmlFor="subcategory-image-upload" className={cn("flex flex-col items-center justify-center w-full h-48 rounded-xl border-2 border-dashed transition-all cursor-pointer", isLoading ? "opacity-60 cursor-not-allowed border-gray-200 dark:border-white/[0.08]" : "border-gray-200 dark:border-white/[0.08] hover:border-[#0F69B0]/40 hover:bg-[#0F69B0]/[0.02]")}>
      ...
      <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 dark:border-white/[0.06]">
        ...
        <button type="submit" disabled={isLoading || categoryOptionsLoading} className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold text-white transition-all cursor-pointer shadow-lg shadow-[#0F69B0]/25 disabled:opacity-60 disabled:cursor-not-allowed" style={{ background: "linear-gradient(135deg, #0F69B0 0%, #0c5a9e 100%)" }}>
          {isLoading ? (<><Loader2 className="h-4 w-4 animate-spin" />Saving...</>) : (<><Save className="h-4 w-4" />{safe.id || safe._id ? "Update Subcategory" : "Create Subcategory"}</>)}
        </button>
      </div>
    </motion.form>
  );
