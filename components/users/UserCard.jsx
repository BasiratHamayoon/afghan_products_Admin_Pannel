...
import {
  Eye, Trash2, ShoppingBag, Package, Shield,
...
const roleConfig = {
  SELLER: {
    label: "Seller",
    gradient: "linear-gradient(135deg, #0F69B0 0%, #0c5a9e 100%)",
    icon: ShoppingBag,
  },
  BUYER: {
    label: "Buyer",
    gradient: "linear-gradient(135deg, #1a82d4 0%, #0F69B0 100%)",
    icon: Package,
  },
  ADMIN: {
    label: "Admin",
    gradient: "linear-gradient(135deg, #0A4F85 0%, #0F69B0 100%)",
    icon: Shield,
  },
};
...
export default function UserCard({ user, index = 0, onView, onDelete, onSuspend }) {
  ...
  return (
    ...
    >
      <div
        className="h-24 w-full relative rounded-t-2xl overflow-hidden"
      ...
      >
        <div className="absolute inset-0">
          ...
          />
          <div
            className="absolute top-3 right-12 w-14 h-14 rounded-full"
      ...
      <div className="relative px-4 pb-4 flex-1 flex flex-col">
        ...
        {user.hasBusiness && (
          <div className="flex items-center justify-center gap-1.5 mb-3 py-2 px-3 rounded-xl bg-[#0F69B0]/[0.04] border border-[#0F69B0]/10">
            ...
            <p className="text-[10px] font-bold text-[#0F69B0]">
              Business Profile
            </p>
          </div>
        )}
        ...
        <div className="flex items-center justify-center gap-2 pt-3 border-t border-gray-100 dark:border-white/[0.06] mt-auto">
          <button
            onClick={() => onView?.(user)}
            className="h-9 w-9 rounded-xl flex items-center justify-center hover:bg-[#0F69B0]/10 text-muted-foreground hover:text-[#0F69B0] transition-all cursor-pointer"
            title="View Details"
          >
          ...
          <button
            ...
            className="h-9 w-9 rounded-xl flex items-center justify-center hover:bg-red-50 dark:hover:bg-red-900/20 text-muted-foreground hover:text-red-500 transition-all cursor-pointer"
            title="Delete"
          >
