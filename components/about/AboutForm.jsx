                ...
                {field.key === "isActive" ? (
                  <div className="flex items-center gap-2 pt-1">
                    <button
                      type="button"
                      onClick={() =>
                        updateItem(index, "isActive", !item.isActive)
                      }
                      disabled={disabled}
                      className={cn(
...
export default function AboutForm({
  ...
  const [metrics, setMetrics] = useState(
    Array.isArray(safe.metrics) ? safe.metrics : []
  );
  const [features, setFeatures] = useState(
    Array.isArray(safe.features) ? safe.features : []
  );
  const [whyChooseUs, setWhyChooseUs] = useState(
    Array.isArray(safe.whyChooseUs) ? safe.whyChooseUs : []
  );
  const [stats, setStats] = useState(
    Array.isArray(safe.stats) ? safe.stats : []
  );
