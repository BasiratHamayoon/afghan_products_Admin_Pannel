...
export default function TradeShowForm({ initialData, onSubmit, onCancel, isLoading }) {
  ...
  const getTagsValue = () => {
    ...
    if (multi && typeof multi === "object") {
      const arr = multi[currentLang] || multi.en || multi.fa || multi.ps || [];
  ...
  const [title, setTitle] = useState(getFieldValue("titleMultilingual", "title"));
  const [description, setDescription] = useState(getFieldValue("descriptionMultilingual", "description"));
  const [country, setCountry] = useState(getFieldValue("countryMultilingual", "country"));
  const [city, setCity] = useState(getFieldValue("cityMultilingual", "city"));
  const [venue, setVenue] = useState(getFieldValue("venueMultilingual", "venue"));
  const [address, setAddress] = useState(getFieldValue("addressMultilingual", "address"));
  const [organizer, setOrganizer] = useState(getFieldValue("organizerMultilingual", "organizer"));
  const [category, setCategory] = useState(getFieldValue("categoryMultilingual", "category"));
  ...
  const handleSubmit = (e) => {
    ...
    formData.append(`country[${lang}]`, country.trim());
    formData.append(`city[${lang}]`, city.trim());
    formData.append(`venue[${lang}]`, venue.trim());
    formData.append(`address[${lang}]`, address.trim());
    formData.append(`organizer[${lang}]`, organizer.trim());
    formData.append(`category[${lang}]`, category.trim());
    formData.append(`tags[${lang}]`, JSON.stringify(tags));
              ...
              <div className="relative w-full h-40 rounded-xl overflow-hidden border border-gray-200 dark:border-white/[0.08] bg-gray-50 dark:bg-white/[0.04]">
                ...
                {imageFile && (
                  <div className="absolute bottom-1 start-1">
                    <p className="text-[9px] text-white bg-black/50 rounded px-1 py-0.5">{imageFile.name}</p>
                  </div>
                )}
              </div>
            )}
                ...
                {galleryItems.map((item, i) => (
                  <div key={i} className="relative group rounded-xl overflow-hidden border border-gray-100 dark:border-white/[0.08] bg-gray-50 dark:bg-white/[0.04] h-28">
                    ...
                    <div className="absolute bottom-1 start-1 end-1">
                      <p className="text-[9px] text-white bg-black/50 rounded px-1 py-0.5 truncate">{item.file?.name || item.url || "Image"}</p>
                    </div>
                  </div>
                ))}
      ...
      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="space-y-1.5">
            ...
            <input type="text" value={country} onChange={(e) => { setCountry(e.target.value); clearErr("country"); }} placeholder={t("tradeShows.countryPlaceholder")} disabled={isLoading} dir={isRTL ? "rtl" : "ltr"} className={inputClass(errors.country)} />
          ...
          <div className="space-y-1.5">
            ...
            <input type="text" value={city} onChange={(e) => { setCity(e.target.value); clearErr("city"); }} placeholder={t("tradeShows.cityPlaceholder")} disabled={isLoading} dir={isRTL ? "rtl" : "ltr"} className={inputClass(errors.city)} />
          ...
          <div className="space-y-1.5">
            ...
            <input type="text" value={venue} onChange={(e) => setVenue(e.target.value)} placeholder={t("tradeShows.venuePlaceholder")} disabled={isLoading} dir={isRTL ? "rtl" : "ltr"} className={inputClass()} />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-foreground uppercase tracking-widest">{t("tradeShows.address")}</label>
            <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder={t("tradeShows.addressPlaceholder")} disabled={isLoading} dir={isRTL ? "rtl" : "ltr"} className={inputClass()} />
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="space-y-1.5">
            ...
            <input type="text" value={organizer} onChange={(e) => setOrganizer(e.target.value)} placeholder={t("tradeShows.organizerNamePlaceholder")} disabled={isLoading} dir={isRTL ? "rtl" : "ltr"} className={inputClass()} />
          </div>
          <div className="space-y-1.5">
