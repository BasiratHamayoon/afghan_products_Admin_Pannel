...
const resolveDate = (dateVal) => {
  ...
  if (typeof dateVal === "object") {
    return dateVal.gregorian || dateVal.display || dateVal.hijri || null;
  }
  return null;
};
...
const normalizeVerification = (v) => {
  ...
  return {
    ...v,
    submittedAt: resolveDate(v.submittedAt),
    reviewedAt: resolveDate(v.reviewedAt),
    createdAt: resolveDate(v.createdAt),
    updatedAt: resolveDate(v.updatedAt),
    expiresAt: resolveDate(v.expiresAt),
  };
};
...
export const fetchVerifications = () => async (dispatch) => {
  ...
  try {
    await delay(600);
    const normalized = (dummyVerifications || []).map(normalizeVerification).filter(Boolean);
    dispatch(setVerifications(normalized));
...
export const fetchVerificationById = (id) => async (dispatch) => {
  ...
  try {
    ...
    dispatch(setSelectedVerification(normalizeVerification(found)));
