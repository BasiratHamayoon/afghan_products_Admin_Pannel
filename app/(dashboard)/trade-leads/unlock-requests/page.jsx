...
import { Unlock, RefreshCw, CheckCircle, XCircle, Clock } from "lucide-react";
...
export default function UnlockRequestsPage() {
  ...
  const { unlockRequests, unlockRequestsLoading, unlockPagination } = useSelector((state) => state.tradeLeads);
  ...
  const triggerFetch = useCallback((page, status) => {
    dispatch(fetchUnlockRequests({ page, limit: PAGE_LIMIT, status: status !== "all" ? status : undefined }));
  }, [dispatch]);
  ...
  return (
    <div className="space-y-5">
      <Breadcrumb />
      <PageHeader title={t("tradeLeads.unlockRequestsTitle")} description={t("tradeLeads.unlockRequestsDesc")} />
      ...
      <div className="rounded-2xl bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)] overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-50 dark:border-white/[0.04]">
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={handleRefresh} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold text-muted-foreground hover:bg-gray-50 dark:hover:bg-white/[0.04] transition-colors cursor-pointer border border-gray-200 dark:border-white/[0.08]" title={t("tradeLeads.refresh")}>
              <RefreshCw className="h-3.5 w-3.5" />{t("tradeLeads.refresh")}
            </button>
            <p className="text-[11px] text-muted-foreground font-medium">
              {safeRequests.length} {safeRequests.length !== 1 ? t("tradeLeads.requestsPlural") : t("tradeLeads.requests")}
            </p>
          </div>
        </div>
        ...
        <div className="p-4">
          {unlockRequestsLoading ? (
            ...
            <EmptyState icon={Unlock} title={t("tradeLeads.noUnlockRequests")} description={t("tradeLeads.noUnlockRequestsDesc")} />
          ) : (
            <>
                        ...
                        >
                          <td className="py-4 px-4">
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              <div className="min-w-0">
                                <p className="text-[11px] font-bold text-foreground truncate max-w-[130px]">{req.sellerName || "—"}</p>
                                <div className="flex items-center gap-1 mt-0.5">
                                  <Mail className="h-3 w-3 text-muted-foreground/50" />
                                  <p className="text-[10px] text-muted-foreground font-medium truncate max-w-[130px]">{req.sellerEmail || "—"}</p>
                                </div>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="py-4 px-4">
                            <div className="flex items-center gap-1.5">
                              ...
                              <button
                                onClick={() => req.tradeLeadId && router.push(`/trade-leads/${req.tradeLeadId}`)}
                                className="text-xs font-medium text-[#0F69B0] hover:underline cursor-pointer truncate max-w-[150px]"
                              >
                                {req.tradeLeadProduct || (req.tradeLeadId ? req.tradeLeadId.toString().slice(0, 12) + "..." : "—")}
                              </button>
                            </div>
                          </td>
                          ...
                          <td className="py-4 px-4">
                            <span className="text-xs font-medium text-muted-foreground whitespace-nowrap">{formatDate(req.createdAt)}</span>
                          </td>
                        </motion.tr>
                      );
                    })}
