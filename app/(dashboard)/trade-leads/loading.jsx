    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] overflow-hidden">
        <div className="flex border-b border-gray-100 dark:border-white/[0.06]">
          {[80, 70, 70, 70].map((w, i) => <div key={i} className="px-5 py-4"><Skeleton className="h-4 rounded" style={{ width: w }} /></div>)}
        </div>
        <div className="p-4 border-b border-gray-50 dark:border-white/[0.04]">
          <div className="flex gap-2"><Skeleton className="h-10 flex-1 rounded-xl" /><Skeleton className="h-10 w-28 rounded-xl" /></div>
        </div>
        <div className="p-4">{[...Array(6)].map((_, i) => <RowSkeleton key={i} />)}</div>
      </div>
    </div>
  );
}