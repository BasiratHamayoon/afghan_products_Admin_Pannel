...
import { ArrowUpCircle, AlertTriangle, DollarSign, SlidersHorizontal, X } from "lucide-react";
...
export default function EscalatedDisputesPage() {
  ...
  const escalatedDisputes = (Array.isArray(disputes) ? disputes.filter(Boolean) : []).filter((d) => d.status === "escalated");

  const { query, setQuery, results: searched } = useSearch(escalatedDisputes, ["title", "orderId", "buyer.name", "seller.name"]);
  ...
  const highPriority = escalatedDisputes.filter((d) => d.priority === "high").length;
  const totalAmount = escalatedDisputes.reduce((acc, d) => acc + (d.amount || 0), 0);

  return (
    <div className="space-y-5">
      <Breadcrumb />
      <PageHeader title="Escalated Disputes" description="High-priority disputes requiring immediate senior attention">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { title: "Escalated", value: escalatedDisputes.length, icon: ArrowUpCircle, color: "rgba(239,68,68,0.08)", iconColor: "#ef4444" },
          { title: "High Priority", value: highPriority, icon: AlertTriangle, color: "rgba(239,68,68,0.08)", iconColor: "#ef4444" },
          { title: "Total Amount", value: totalAmount, prefix: "$", icon: DollarSign, color: "rgba(245,158,11,0.08)", iconColor: "#f59e0b" },
        ].map((stat, i) => (
          <motion.div
            key={stat.title}
      ...
      <div className="rounded-2xl bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)] overflow-hidden">
        <div className="p-4 border-b border-gray-50 dark:border-white/[0.04] space-y-3">
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex-1 min-w-[180px]">
              <SearchInput value={query} onChange={setQuery} placeholder="Search escalated disputes..." />
            </div>
            <button
        ...
        <div className="p-4">
          {isLoading ? (
            <LoadingSpinner size="lg" text="Loading escalated disputes..." className="py-16" />
          ) : filteredData.length === 0 ? (
            <EmptyState icon={ArrowUpCircle} title="No escalated disputes" description={hasActiveFilters ? "Try adjusting your filters" : "No disputes have been escalated"} />
          ) : (
            <>
