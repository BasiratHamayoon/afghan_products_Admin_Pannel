...
import { Clock, AlertTriangle, DollarSign, SlidersHorizontal, X } from "lucide-react";
...
export default function OpenDisputesPage() {
  ...
  const openDisputes = (Array.isArray(disputes) ? disputes.filter(Boolean) : []).filter((d) => d.status === "open");

  const { query, setQuery, results: searched } = useSearch(openDisputes, ["title", "orderId", "buyer.name", "seller.name"]);
  ...
  const highPriority = openDisputes.filter((d) => d.priority === "high").length;
  const totalAmount = openDisputes.reduce((acc, d) => acc + (d.amount || 0), 0);

  return (
    <div className="space-y-5">
      <Breadcrumb />
      <PageHeader title="Open Disputes" description="All currently open disputes awaiting resolution">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { title: "Open Disputes", value: openDisputes.length, icon: Clock, color: "rgba(15,105,176,0.08)", iconColor: "#0F69B0" },
      ...
      <div className="rounded-2xl bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)] overflow-hidden">
        <div className="p-4 border-b border-gray-50 dark:border-white/[0.04] space-y-3">
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex-1 min-w-[180px]">
              <SearchInput value={query} onChange={setQuery} placeholder="Search open disputes..." />
            </div>
            <button
        ...
        <div className="p-4">
          {isLoading ? (
            <LoadingSpinner size="lg" text="Loading open disputes..." className="py-16" />
          ) : filteredData.length === 0 ? (
            <EmptyState icon={Clock} title="No open disputes" description={hasActiveFilters ? "Try adjusting your filters" : "All disputes have been resolved"} />
          ) : (
            <>
