...
function AddStoryContent() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        else { toast.error("Failed to load story"); setInitialData({}); }
  ...
  const handleSubmit = async (payload) => {
    ...
    try {
      ...
      if (res?.success) {
        toast.success(isEditMode ? "Story updated!" : "Story created!");
  ...
  if (isFetching || initialData === null) {
    return <div className="flex items-center justify-center py-32"><div className="flex flex-col items-center gap-3"><Loader2 className="h-8 w-8 animate-spin text-[#0F69B0]" /><p className="text-sm text-muted-foreground font-medium">{isEditMode ? "Loading story..." : "Preparing form..."}</p></div></div>;
  }

  const title = isEditMode ? "Edit Success Story" : "Add Success Story";
  const desc = isEditMode ? "Update story details" : "Create a new success story";

  return (
    <div className="space-y-5">
      <Breadcrumb />
      <PageHeader title={title} description={desc}>
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => router.push("/success-stories")} className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-gray-200 dark:border-white/[0.08] text-sm font-bold text-muted-foreground hover:bg-gray-50 dark:hover:bg-white/[0.04] transition-colors cursor-pointer"><ArrowLeft className="h-4 w-4" />Back</motion.button>
      </PageHeader>
