...
function RowSkeleton() {
  return (
    <div className="flex items-center gap-4 py-4 px-4 border-b border-gray-50 dark:border-white/[0.03] last:border-0">
      ...
      <Skeleton className="h-3.5 w-12 hidden xl:block" />
      <Skeleton className="h-6 w-16 rounded-full" />
      <div className="flex gap-1"><Skeleton className="h-8 w-20 rounded-lg" /><Skeleton className="h-8 w-16 rounded-lg" /></div>
    </div>
  );
}
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] overflow-hidden">
        <div className="flex border-b border-gray-100 dark:border-white/[0.06]">
          {[80, 70, 110].map((w, i) => <div key={i} className="px-4 py-4"><Skeleton className="h-4 rounded" style={{ width: w }} /></div>)}
        </div>
        <div className="p-4 border-b border-gray-50 dark:border-white/[0.04]">
          <div className="flex gap-2"><Skeleton className="h-10 flex-1 rounded-xl" /><Skeleton className="h-10 w-20 rounded-xl" /></div>
        </div>
