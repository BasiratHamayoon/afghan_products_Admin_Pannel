...
function AddFaqContent() {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = { question: question.trim(), answer: answer.trim(), category: category.trim(), order, isActive };
    setIsLoading(true);
    try {
      const res = isEditMode ? await dispatch(updateFaq(editId, payload)) : await dispatch(addFaq(payload));
      if (res?.success) {
        toast.success(isEditMode ? "FAQ updated!" : "FAQ added!");
  ...
  return (
    <div className="space-y-5">
      <Breadcrumb />
      <PageHeader title={isEditMode ? "Edit FAQ" : "Add FAQ"} description="Manage FAQ entries">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)] p-6">
        <div className="flex items-center gap-3 mb-6 pb-5 border-b border-gray-100 dark:border-white/[0.06]">
          <div className="h-11 w-11 rounded-xl flex items-center justify-center" style={{ background: "rgba(15,105,176,0.1)" }}><MessageSquare className="h-5 w-5 text-[#0F69B0]" /></div>
          <div><h2 className="text-base font-black text-foreground">{isEditMode ? "Edit FAQ" : "Add FAQ"}</h2></div>
        </div>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-foreground uppercase tracking-widest">Question <span className="text-red-500">*</span></label>
            <input type="text" value={question} onChange={(e) => { setQuestion(e.target.value); if (errors.question) setErrors((p) => ({ ...p, question: "" })); }} placeholder="e.g. How do I create an account?" disabled={isLoading} className={inputClass(errors.question)} />
            {errors.question && <p className="text-[11px] text-red-500 font-semibold">{errors.question}</p>}
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-foreground uppercase tracking-widest">Answer <span className="text-red-500">*</span></label>
            <textarea value={answer} onChange={(e) => { setAnswer(e.target.value); if (errors.answer) setErrors((p) => ({ ...p, answer: "" })); }} rows={5} placeholder="Answer..." disabled={isLoading} className={cn(inputClass(errors.answer), "resize-none")} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div className="space-y-1.5">
              ...
              <input type="text" value={category} onChange={(e) => setCategory(e.target.value)} placeholder="e.g. Account" disabled={isLoading} className={inputClass()} />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 dark:border-white/[0.06]">
            <button type="button" onClick={() => router.push("/help-center")} disabled={isLoading} className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-gray-200 dark:border-white/[0.08] text-sm font-bold text-muted-foreground hover:bg-gray-50 dark:hover:bg-white/[0.04] cursor-pointer disabled:opacity-60"><X className="h-4 w-4" />Cancel</button>
            <button type="submit" disabled={isLoading} className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold text-white cursor-pointer shadow-lg shadow-[#0F69B0]/25 disabled:opacity-60" style={{ background: "linear-gradient(135deg, #0F69B0 0%, #0c5a9e 100%)" }}>
              {isLoading ? <><Loader2 className="h-4 w-4 animate-spin" />Saving...</> : <><Save className="h-4 w-4" />{isEditMode ? "Update" : "Add"} FAQ</>}
            </button>
          </div>
        </form>
      </motion.div>
