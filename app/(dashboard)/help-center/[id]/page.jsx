...
import {
  ...
  HelpCircle, Calendar, FileText, MessageSquare,
  FolderOpen, Phone,
} from "lucide-react";
...
function DetailContent() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (res?.success) { toast.success("Deleted"); clearHelpCenterByIdCache(itemData.id); dispatch(fetchHelpCenter()); router.push("/help-center"); }
  ...
  if (notFound || !itemData) return (
    <div className="flex flex-col items-center justify-center py-32 gap-4">
      ...
      <h2 className="text-lg font-black text-foreground">Not found</h2>
      <button onClick={() => router.push("/help-center")} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold text-white cursor-pointer" style={{ background: "linear-gradient(135deg, #0F69B0 0%, #0c5a9e 100%)" }}><ArrowLeft className="h-4 w-4" />Back</button>
    </div>
  );

  const fields = [
    { label: "Header Title", value: itemData.headerTitle || "—", icon: FileText },
    { label: "Header Subtitle", value: itemData.headerSubtitle || "—", icon: FileText },
    { label: "Hero Title", value: itemData.heroTitle || "—", icon: FileText },
    { label: "Support Title", value: itemData.supportTitle || "—", icon: FileText },
    { label: "Status", value: itemData.isActive ? "Active" : "Inactive", icon: HelpCircle, isStatus: true },
  ...
  return (
    <div className="space-y-5">
      <Breadcrumb />
      <PageHeader title="Help Center Detail" description={itemData.headerTitle || ""}>
        <div className="flex items-center gap-2">
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => router.push("/help-center")} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/[0.08] text-sm font-bold text-muted-foreground hover:bg-gray-50 dark:hover:bg-white/[0.04] cursor-pointer"><ArrowLeft className="h-4 w-4" />Back</motion.button>
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => router.push(`/help-center/add?edit=${id}`)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-white cursor-pointer shadow-lg shadow-[#0F69B0]/25" style={{ background: "linear-gradient(135deg, #0F69B0 0%, #0c5a9e 100%)" }}><Edit2 className="h-4 w-4" />Edit</motion.button>
        </div>
      </PageHeader>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)] flex flex-col items-center text-center">
          <div className="h-20 w-20 rounded-2xl flex items-center justify-center mb-4" style={{ background: "rgba(15,105,176,0.1)" }}><HelpCircle className="h-10 w-10 text-[#0F69B0]" /></div>
          <h3 className="text-lg font-black text-foreground">{itemData.headerTitle}</h3>
          <span className={cn("text-[11px] font-bold px-3 py-1 rounded-full mt-3", itemData.isActive ? "bg-emerald-500/10 text-emerald-600" : "bg-gray-500/10 text-gray-500")}>{itemData.isActive ? "Active" : "Inactive"}</span>
          <div className="grid grid-cols-3 gap-2 mt-4 w-full">
            <div className="p-2 rounded-xl bg-gray-50 dark:bg-white/[0.03] text-center"><p className="text-lg font-black">{itemData.faqs?.length || 0}</p><p className="text-[9px] font-bold text-muted-foreground uppercase">FAQs</p></div>
            <div className="p-2 rounded-xl bg-gray-50 dark:bg-white/[0.03] text-center"><p className="text-lg font-black">{itemData.categories?.length || 0}</p><p className="text-[9px] font-bold text-muted-foreground uppercase">Cats</p></div>
            <div className="p-2 rounded-xl bg-gray-50 dark:bg-white/[0.03] text-center"><p className="text-lg font-black">{itemData.contactOptions?.length || 0}</p><p className="text-[9px] font-bold text-muted-foreground uppercase">Contacts</p></div>
          </div>
          <div className="flex items-center gap-2 mt-5 pt-4 border-t border-gray-100 dark:border-white/[0.06] w-full justify-center flex-wrap">
            <button onClick={() => router.push(`/help-center/add?edit=${id}`)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-[#0F69B0] hover:bg-[#0F69B0]/[0.08] cursor-pointer border border-[#0F69B0]/20"><Edit2 className="h-3.5 w-3.5" />Edit</button>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="lg:col-span-2 rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)]">
          ...
          {itemData.heroDescription && (
            <div className="mt-4 p-4 rounded-xl border border-gray-100 dark:border-white/[0.06] bg-gray-50/50 dark:bg-white/[0.02]">
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Hero Description</p>
              <p className="text-sm text-foreground font-medium leading-relaxed">{itemData.heroDescription}</p>
            </div>
          )}
          {itemData.supportDescription && (
            <div className="mt-4 p-4 rounded-xl border border-gray-100 dark:border-white/[0.06] bg-gray-50/50 dark:bg-white/[0.02]">
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">Support Description</p>
      ...
      <ConfirmDialog open={deleteDialog} onClose={() => setDeleteDialog(false)} onConfirm={handleDeleteConfirm} title="Delete Help Center" description={`Delete "${itemData?.headerTitle}"? This cannot be undone.`} confirmLabel="Delete" isLoading={isDeleting} variant="danger" />
    </div>
  );
}
