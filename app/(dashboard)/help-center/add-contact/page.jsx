...
function AddContactContent() {
  ...
  return (
    <div className="space-y-5">
      <Breadcrumb />
      <PageHeader title={isEditMode ? "Edit Contact Option" : "Add Contact Option"} description="Manage contact options">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)] p-6">
        <div className="flex items-center gap-3 mb-6 pb-5 border-b border-gray-100 dark:border-white/[0.06]">
          ...
          <h2 className="text-base font-black text-foreground">{isEditMode ? "Edit Contact Option" : "Add Contact Option"}</h2>
        </div>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div className="space-y-1.5">
              ...
              <input type="text" value={title} onChange={(e) => { setTitle(e.target.value); if (errors.title) setErrors((p) => ({ ...p, title: "" })); }} placeholder="e.g. Email Support" disabled={isLoading} className={inputClass(errors.title)} />
            ...
            <div className="space-y-1.5">
              ...
              <input type="text" value={icon} onChange={(e) => setIcon(e.target.value)} placeholder="e.g. Mail, Phone" disabled={isLoading} className={inputClass()} />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-foreground uppercase tracking-widest">Type</label>
              <input type="text" value={type} onChange={(e) => setType(e.target.value)} placeholder="e.g. email, phone, chat" disabled={isLoading} className={inputClass()} />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-foreground uppercase tracking-widest">Value</label>
              <input type="text" value={value} onChange={(e) => setValue(e.target.value)} placeholder="e.g. support@example.com" disabled={isLoading} className={inputClass()} />
            </div>
