...
function AddCategoryContent() {
  ...
  useEffect(() => {
    ...
    if (cat) {
      setTitle(getFieldValue(cat, "titleMultilingual", "title"));
      setDescription(getFieldValue(cat, "descriptionMultilingual", "description"));
      setIcon(cat.icon || "");
      setLink(cat.link || "");
      setOrder(cat.order ?? 0);
      setIsActive(cat.isActive ?? true);
    }
  }, [editId, categories, currentLang]);
  ...
  const handleSubmit = async (e) => {
    ...
    const errs = {};
    if (!title.trim()) errs.title = t("helpCenter.categoryTitleRequired");
    ...
    if (description.trim())
      payload.description = { [lang]: description.trim() };
  ...
  const pageTitle = isEditMode
    ? t("helpCenter.editCategoryTitle")
    : t("helpCenter.addCategoryTitle");
    ...
    <div className="space-y-5">
      ...
      <PageHeader title={pageTitle} description={t("helpCenter.categoryFormDesc")}>
        <motion.button
          ...
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-gray-200 dark:border-white/[0.08] text-sm font-bold text-muted-foreground hover:bg-gray-50 dark:hover:bg-white/[0.04] cursor-pointer"
        >
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-foreground uppercase tracking-widest">
                {t("helpCenter.categoryTitleLabel")}{" "}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-foreground uppercase tracking-widest">
                {t("helpCenter.iconLabel")}
              </label>
              <input
                type="text"
                value={icon}
                onChange={(e) => setIcon(e.target.value)}
                placeholder={t("helpCenter.iconPlaceholder")}
                disabled={isLoading}
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 dark:border-white/[0.06]">
            <button
              ...
              onClick={() => router.push("/help-center")}
              disabled={isLoading}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-gray-200 dark:border-white/[0.08] text-sm font-bold text-muted-foreground cursor-pointer disabled:opacity-60"
            >
              <X className="h-4 w-4" />
            ...
            >
              {isLoading ? (
                ...
                <>
                  ...
                  {isEditMode
                    ? t("helpCenter.updateCategoryBtn")
                    : t("helpCenter.addCategoryBtn")}
                </>
              )}
            </button>
          </div>
