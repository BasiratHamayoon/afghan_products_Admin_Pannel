...
function AddEditSectionContent() {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (res?.success && res?.data) {
          ...
          const existingIds = (res.data.products || []).map(
            (p) => p._id || p.id || p
          );
  ...
  const handleSaveProducts = async () => {
    ...
    try {
      const res = await dispatch(
        manageSectionProducts(sectionData.id, selectedProductIds)
      );
      if (res?.success) {
  ...
  const description = isProductsMode
    ? `Select products for "${sectionData?.name || "section"}"`
    : isEditMode
      ? "Update section details"
      : "Create a new homepage section";

  const TitleIcon = isProductsMode
    ? Package
    : isEditMode
      ? Edit2
      : LayoutGrid;
            ...
            {selectedProductIds.length > 0 && (
              <div className="flex items-center gap-2 mb-4 p-3 rounded-xl bg-[#0F69B0]/[0.05] border border-[#0F69B0]/20">
                ...
                <p className="text-xs font-semibold text-[#0F69B0]">
                  {selectedProductIds.length} product
                  {selectedProductIds.length !== 1 ? "s" : ""} selected.
                  Click "Save Products" to apply changes.
                </p>
              </div>
            )}

            <div className="relative mb-4">
              <Search className="absolute left-3.5 h-4 w-4 text-muted-foreground/50 pointer-events-none top-1/2 -translate-y-1/2" />
              <input
                type="text"
                value={productSearch}
                ...
                {filteredProducts.map((product) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex items-center gap-3 p-3 rounded-xl border-2 transition-all cursor-pointer text-left w-full",
                        isSelected
