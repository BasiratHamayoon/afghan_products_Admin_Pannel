// app/(dashboard)/investments/active/page.jsx
...
export default function ActiveInvestmentsPage() {
  ...
  useEffect(() => {
    setTimeout(() => {
      setData(dummyInvestments.filter((inv) => inv.status === "active"));
      setIsLoading(false);
    }, 600);
  }, []);
  ...
  return (
    <div>
      <Breadcrumb />
      <PageHeader title="Active Investments" description="Currently active investment opportunities">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatsCard title="Active Count" value={data.length} icon={TrendingUp} color="rgba(16,185,129,0.08)" index={0} />
        ...
        <StatsCard title="Avg ROI" value={`${avgRoi}%`} icon={TrendingUp} color="rgba(245,158,11,0.08)" index={3} />
      </div>
      ...
      >
        <div className="p-5 border-b border-gray-50 dark:border-white/[0.04]">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <SearchInput value={query} onChange={setQuery} placeholder="Search active investments..." className="flex-1" />
        ...
        <div className="p-5">
          {isLoading ? (
            <LoadingSpinner size="lg" text="Loading active investments..." className="py-16" />
