...
import { fetchConsultantById, deleteConsultant, clearConsultantByIdCache } from "@/store/actions/consultancyActions";
...
function DetailContent() {
  ...
  const DAYS_LABEL = {
    mon: t("consultancy.mon"),
    tue: t("consultancy.tue"),
    wed: t("consultancy.wed"),
    thu: t("consultancy.thu"),
    fri: t("consultancy.fri"),
    sat: t("consultancy.sat"),
    sun: t("consultancy.sun"),
  };
    ...
    return (
      <div className="flex items-center justify-center py-32">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground font-medium">{t("consultancy.loadingConsultant")}</p>
        </div>
      </div>
    );
  }
  ...
  if (notFound || !itemData) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4">
        ...
        <h2 className="text-lg font-black text-foreground">{t("consultancy.consultantNotFound")}</h2>
        <button onClick={() => router.push("/consultancy")} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold text-white cursor-pointer" style={{ background: "linear-gradient(135deg, #0F69B0 0%, #0c5a9e 100%)" }}>
          <ArrowLeft className="h-4 w-4 rtl-mirror" />{t("consultancy.back")}
        </button>
      </div>
    );
  }
  ...
  const detailFields = [
    { label: t("consultancy.nameField"), value: itemData.name, icon: FileText },
    { label: t("consultancy.titleField"), value: itemData.title, icon: FileText },
    { label: t("consultancy.specializationField"), value: itemData.specialization || "—", icon: GraduationCap },
    { label: t("consultancy.minRate"), value: `$${itemData.hourlyRateMin}/hr`, icon: DollarSign },
    { label: t("consultancy.maxRate"), value: `$${itemData.hourlyRateMax}/hr`, icon: DollarSign },
    { label: t("consultancy.totalSessionsField"), value: String(itemData.totalSessions ?? 0), icon: Hash },
    { label: t("consultancy.statusField"), value: itemData.isActive ? t("consultancy.activeStatus") : t("consultancy.inactiveStatus"), icon: CheckCircle, isStatus: true },
    { label: t("consultancy.createdField"), value: formatDate(itemData.createdAt), icon: Calendar },
    { label: t("consultancy.updatedField"), value: formatDate(itemData.updatedAt), icon: Calendar },
  ];

  return (
    <div className="space-y-5">
      <Breadcrumb />
      <PageHeader title={t("consultancy.consultantDetail")} description={itemData.name || ""}>
        <div className="flex items-center gap-2">
          ...
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => router.push(`/consultancy/add?edit=${id}`)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-white cursor-pointer shadow-lg shadow-[#0F69B0]/25" style={{ background: "linear-gradient(135deg, #0F69B0 0%, #0c5a9e 100%)" }}>
            <Edit2 className="h-4 w-4" />{t("consultancy.edit")}
          </motion.button>
        </div>
      </PageHeader>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)] flex flex-col items-center text-center">
          <div className="h-24 w-24 rounded-full overflow-hidden border-2 border-gray-100 dark:border-white/[0.08] bg-gray-50 dark:bg-white/[0.04] flex items-center justify-center mb-4">
            {imgUrl
              ? <img src={imgUrl} alt={itemData.name} className="object-cover w-full h-full" />
              : <span className="text-3xl font-black text-muted-foreground">{itemData.name?.charAt(0) || "?"}</span>
            }
          </div>
          ...
          <p className="text-xs text-muted-foreground font-medium mt-1">
            {Number(itemData.rating).toFixed(1)} {t("consultancy.ratingLabel")} · {itemData.totalSessions} {t("consultancy.sessionsLabel")}
          </p>
          ...
          <span className={cn("text-[11px] font-bold px-3 py-1 rounded-full mt-3", itemData.isActive ? "bg-emerald-500/10 text-emerald-600" : "bg-gray-500/10 text-gray-500")}>
            {itemData.isActive ? t("consultancy.activeStatus") : t("consultancy.inactiveStatus")}
          </span>
          ...
          <div className="flex items-center gap-2 mt-5 pt-4 border-t border-gray-100 dark:border-white/[0.06] w-full justify-center flex-wrap">
            <button onClick={() => router.push(`/consultancy/add?edit=${id}`)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-[#0F69B0] hover:bg-[#0F69B0]/[0.08] cursor-pointer border border-[#0F69B0]/20">
              <Edit2 className="h-3.5 w-3.5" />{t("consultancy.edit")}
            </button>
            <button onClick={() => setDeleteDialog(true)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 cursor-pointer border border-red-200 dark:border-red-800/40">
              <Trash2 className="h-3.5 w-3.5" />{t("consultancy.delete")}
            </button>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="lg:col-span-2 space-y-5">
          <div className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)]">
            <h3 className="text-sm font-black text-foreground mb-5">{t("consultancy.consultantInformation")}</h3>
          ...
          {itemData.description && (
            <div className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)]">
              <h3 className="text-sm font-black text-foreground mb-4">{t("consultancy.descriptionSection")}</h3>
          ...
          {itemData.availability?.length > 0 && (
            <div className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)]">
              <h3 className="text-sm font-black text-foreground mb-4">{t("consultancy.availabilitySection")}</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      ...
      <ConfirmDialog
        ...
        description={`${t("consultancy.deleteConsultantDesc")} "${itemData?.name}"${t("consultancy.deleteConsultantSuffix")}`}
