...
import {
  ...
  Loader2, XCircle, Calendar, Mail, Phone, User, Hash, FileText,
} from "lucide-react";
...
function DetailContent() {
  ...
  const handleArchiveConfirm = async () => {
    ...
    try {
      ...
      if (res?.success) {
        toast.success(archiveDialog.action === "archive" ? t("contactUs.messageArchived") : t("contactUs.messageUnarchived"));
  ...
  const handleMarkViewed = async () => {
    ...
    try {
      ...
      if (res?.success) {
        toast.success(t("contactUs.markedAsRead"));
        setItemData((prev) => prev ? { ...prev, status: "READ", isViewed: true } : prev);
      } else {
        toast.error(res?.message || t("contactUs.actionFailed"));
      }
  ...
  if (notFound || !itemData) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4">
        ...
        <h2 className="text-lg font-black text-foreground">{t("contactUs.messageNotFound")}</h2>
        <p className="text-sm text-muted-foreground font-medium">{t("contactUs.messageNotFoundDesc")}</p>
        <button onClick={() => router.push("/contact-us")} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold text-white cursor-pointer" style={{ background: "linear-gradient(135deg, #0F69B0 0%, #0c5a9e 100%)" }}>
  ...
  const detailFields = [
    { label: t("contactUs.nameField"), value: itemData.name, icon: User },
    { label: t("contactUs.emailField"), value: itemData.email, icon: Mail },
    { label: t("contactUs.phoneField"), value: itemData.phone || "—", icon: Phone },
    { label: t("contactUs.subjectField"), value: itemData.subject || t("contactUs.noSubject"), icon: FileText },
    { label: t("contactUs.statusField"), value: status.label, icon: Hash, isStatus: true },
    { label: t("contactUs.receivedField"), value: formatDate(itemData.createdAt), icon: Calendar },
    { label: t("contactUs.updatedField"), value: formatDate(itemData.updatedAt), icon: Calendar },
  ];

  return (
    <div className="space-y-5">
      <Breadcrumb />
      <PageHeader title={t("contactUs.messageDetail")} description={itemData.name || ""}>
        <div className="flex items-center gap-2 flex-wrap">
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => router.push("/contact-us")} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/[0.08] text-sm font-bold text-muted-foreground hover:bg-gray-50 dark:hover:bg-white/[0.04] transition-colors cursor-pointer">
            <ArrowLeft className="h-4 w-4 rtl-mirror" />{t("contactUs.back")}
          </motion.button>
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => router.push(`/contact-us/reply?id=${id}`)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-white cursor-pointer shadow-lg shadow-[#0F69B0]/25" style={{ background: "linear-gradient(135deg, #0F69B0 0%, #0c5a9e 100%)" }}>
            <Reply className="h-4 w-4" />{t("contactUs.reply")}
          </motion.button>
        </div>
      </PageHeader>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)] flex flex-col items-center text-center">
          <div className="h-20 w-20 rounded-full flex items-center justify-center text-2xl font-black text-white mb-4" style={{ background: "linear-gradient(135deg, #0F69B0 0%, #0c5a9e 100%)" }}>
            {getInitials(itemData.name)}
          </div>
          <h3 className="text-lg font-black text-foreground mb-1">{itemData.name || "Unknown"}</h3>
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground font-medium">
            <Mail className="h-3.5 w-3.5" />{itemData.email || "—"}
          </div>
          {itemData.phone && (
            <div className="flex items-center gap-1.5 text-xs text-muted-foreground font-medium mt-1">
              <Phone className="h-3.5 w-3.5" />{itemData.phone}
            </div>
          )}

          <div className="flex items-center gap-2 mt-4 flex-wrap justify-center">
            <span className={cn("text-[11px] font-bold px-3 py-1 rounded-full", status.bg, status.text)}>{status.label}</span>
            {itemData.isArchived && (
              <span className="text-[11px] font-bold px-3 py-1 rounded-full bg-amber-500/10 text-amber-600">{t("contactUs.archivedBadge")}</span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 mt-5 pt-4 border-t border-gray-100 dark:border-white/[0.06] w-full justify-center flex-wrap">
            <button onClick={() => router.push(`/contact-us/reply?id=${id}`)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-[#0F69B0] hover:bg-[#0F69B0]/[0.08] transition-colors cursor-pointer border border-[#0F69B0]/20">
              <Reply className="h-3.5 w-3.5" />{t("contactUs.reply")}
            </button>
            {itemData.status === "UNREAD" && (
              <button onClick={handleMarkViewed} disabled={isActioning} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-gray-600 hover:bg-gray-100 dark:hover:bg-white/[0.06] transition-colors cursor-pointer border border-gray-200 dark:border-white/[0.08] disabled:opacity-60">
                <CheckCheck className="h-3.5 w-3.5" />{t("contactUs.markRead")}
              </button>
            )}
            {itemData.isArchived ? (
              <button onClick={() => setArchiveDialog({ open: true, action: "unarchive" })} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors cursor-pointer border border-emerald-200 dark:border-emerald-800/40">
                <ArchiveRestore className="h-3.5 w-3.5" />{t("contactUs.unarchive")}
              </button>
            ) : (
              <button onClick={() => setArchiveDialog({ open: true, action: "archive" })} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors cursor-pointer border border-amber-200 dark:border-amber-800/40">
                <Archive className="h-3.5 w-3.5" />{t("contactUs.archive")}
              </button>
            )}
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="lg:col-span-2 space-y-5">
          <div className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)]">
            <h3 className="text-sm font-black text-foreground mb-5">{t("contactUs.contactInformation")}</h3>
                    ...
                    <div className="min-w-0">
                      ...
                      {field.isStatus ? (
                        <span className={cn("inline-flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-lg", status.bg, status.text)}>
                          <span className={cn("h-1.5 w-1.5 rounded-full", status.dot)} />{field.value}
                        </span>
          ...
          <div className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)]">
            <h3 className="text-sm font-black text-foreground mb-4">{t("contactUs.messageSection")}</h3>
            <div className="p-4 rounded-xl border border-gray-100 dark:border-white/[0.06] bg-gray-50/50 dark:bg-white/[0.02]">
              <p className="text-sm text-foreground font-medium leading-relaxed whitespace-pre-wrap">{itemData.message || t("contactUs.noMessageContent")}</p>
            </div>
          </div>
          ...
          {itemData.adminReply && (
            <div className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-emerald-100 dark:border-emerald-900/30 shadow-[0_2px_12px_rgba(16,185,129,0.06)]">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-black text-emerald-700 dark:text-emerald-400">{t("contactUs.adminReply")}</h3>
                {itemData.repliedAt && (
                  <span className="ml-auto text-[11px] text-muted-foreground font-medium">{formatDate(itemData.repliedAt)}</span>
                )}
              </div>
              <div className="p-4 rounded-xl border border-emerald-100 dark:border-emerald-900/20 bg-emerald-50/50 dark:bg-emerald-900/10">
                <p className="text-sm text-foreground font-medium leading-relaxed whitespace-pre-wrap">{itemData.adminReply}</p>
              </div>
            </div>
          )}
        </motion.div>
      ...
      <ConfirmDialog
        ...
        description={`${archiveDialog.action === "archive" ? t("contactUs.archiveDesc") : t("contactUs.unarchiveDesc")} "${itemData?.name}"?`}
