...
import { fetchTradeShowById, deleteTradeShow, clearTradeShowByIdCache } from "@/store/actions/tradeShowsActions";
...
function DetailContent() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      } else toast.error(res?.message || t("tradeShows.failedToDelete"));
    } catch { toast.error(t("tradeShows.somethingWentWrong")); }
    ...
    return (
      <div className="flex items-center justify-center py-32">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground font-medium">{t("tradeShows.loadingTradeShow")}</p>
        </div>
      </div>
    );
  }
  ...
  if (notFound || !itemData) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4">
        ...
        <h2 className="text-lg font-black text-foreground">{t("tradeShows.tradeShowNotFound")}</h2>
        <button
        ...
        >
          <ArrowLeft className="h-4 w-4 rtl-mirror" />{t("tradeShows.backToTradeShows")}
        </button>
      </div>
    );
  }
  ...
  const getStatus = () => {
    if (!itemData.isActive) return { label: t("tradeShows.statusInactive"), cls: "bg-gray-500/10 text-gray-500" };
    const now = new Date();
    const s = new Date(itemData.startDate);
    const e = new Date(itemData.endDate);
    if (now < s) return { label: t("tradeShows.statusUpcoming"), cls: "bg-blue-500/10 text-blue-600" };
    if (now >= s && now <= e) return { label: t("tradeShows.statusOngoing"), cls: "bg-emerald-500/10 text-emerald-600" };
    return { label: t("tradeShows.statusEnded"), cls: "bg-amber-500/10 text-amber-600" };
  };
  const status = getStatus();

  const detailFields = [
    { label: t("tradeShows.titleField"), value: itemData.title, icon: FileText },
    { label: t("tradeShows.countryField"), value: itemData.country, icon: MapPin },
    { label: t("tradeShows.cityField"), value: itemData.city, icon: MapPin },
    { label: t("tradeShows.venueField"), value: itemData.venue || "—", icon: MapPin },
    { label: t("tradeShows.addressField"), value: itemData.address || "—", icon: MapPin },
    { label: t("tradeShows.startDateField"), value: formatDate(itemData.startDate), icon: Calendar },
    { label: t("tradeShows.endDateField"), value: formatDate(itemData.endDate), icon: Calendar },
    { label: t("tradeShows.organizerField"), value: itemData.organizer || "—", icon: User },
    { label: t("tradeShows.emailField"), value: itemData.organizerEmail || "—", icon: Mail },
    { label: t("tradeShows.phoneField"), value: itemData.organizerPhone || "—", icon: Phone },
    { label: t("tradeShows.websiteField"), value: itemData.website || "—", icon: Globe },
    { label: t("tradeShows.categoryField"), value: itemData.category || "—", icon: Tag },
    { label: t("tradeShows.createdField"), value: formatDate(itemData.createdAt), icon: Calendar },
    { label: t("tradeShows.updatedField"), value: formatDate(itemData.updatedAt), icon: Calendar },
  ];

  return (
    <div className="space-y-5">
      <Breadcrumb />
      <PageHeader title={t("tradeShows.tradeShowDetail")} description={itemData.title || ""}>
        <div className="flex items-center gap-2">
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => router.push("/trade-shows")} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/[0.08] text-sm font-bold text-muted-foreground hover:bg-gray-50 dark:hover:bg-white/[0.04] cursor-pointer">
            <ArrowLeft className="h-4 w-4 rtl-mirror" />{t("tradeShows.back")}
          </motion.button>
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => router.push(`/trade-shows/add?edit=${id}`)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-white cursor-pointer shadow-lg shadow-[#0F69B0]/25" style={{ background: "linear-gradient(135deg, #0F69B0 0%, #0c5a9e 100%)" }}>
            <Edit2 className="h-4 w-4" />{t("tradeShows.edit")}
          </motion.button>
        </div>
      </PageHeader>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)] flex flex-col items-center text-center">
          ...
          <div className="flex items-center gap-2 mt-4 flex-wrap justify-center">
            ...
            {itemData.isFeatured && (
              <span className="text-[11px] font-bold px-3 py-1 rounded-full bg-amber-500/10 text-amber-600 flex items-center gap-1">
                <Star className="h-3 w-3 fill-amber-500" />{t("tradeShows.featuredBadge")}
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 mt-5 pt-4 border-t border-gray-100 dark:border-white/[0.06] w-full justify-center flex-wrap">
            <button onClick={() => router.push(`/trade-shows/add?edit=${id}`)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-[#0F69B0] hover:bg-[#0F69B0]/[0.08] cursor-pointer border border-[#0F69B0]/20">
              <Edit2 className="h-3.5 w-3.5" />{t("tradeShows.edit")}
            </button>
            <button onClick={() => setDeleteDialog(true)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 cursor-pointer border border-red-200 dark:border-red-800/40">
              <Trash2 className="h-3.5 w-3.5" />{t("tradeShows.delete")}
            </button>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="lg:col-span-2 space-y-5">
          <div className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)]">
            <h3 className="text-sm font-black text-foreground mb-5">{t("tradeShows.tradeShowInformation")}</h3>
          ...
          {itemData.description && (
            <div className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)]">
              <h3 className="text-sm font-black text-foreground mb-4">{t("tradeShows.descriptionSection")}</h3>
          ...
          {hasGallery && (
            <div className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)]">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-black text-foreground">{t("tradeShows.gallerySection")} ({itemData.gallery.length})</h3>
              </div>
      ...
      <ConfirmDialog
        ...
        description={`${t("tradeShows.deleteTradeShowDesc")} "${itemData?.title}"${t("tradeShows.deleteTradeShowSuffix")}`}
