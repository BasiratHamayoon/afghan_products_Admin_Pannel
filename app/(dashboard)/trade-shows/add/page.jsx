...
function AddTradeShowContent() {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (res?.success) {
          ...
          toast.error("Failed to load trade show");
          setInitialData({});
        }
      } catch {
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      ...
      if (res?.success) {
        toast.success(
          isEditMode
            ? "Trade show updated successfully!"
            : "Trade show created successfully!"
        );
        router.push("/trade-shows");
      } else {
        toast.error(
          res?.message ||
            `Failed to ${isEditMode ? "update" : "create"} trade show`
        );
      }
    } catch {
      toast.error("Something went wrong");
  ...
  const title = isEditMode ? "Edit Trade Show" : "Add Trade Show";
  const desc = isEditMode
    ? "Update trade show details"
    : "Create a new trade show event";
