...
import { createBanner } from "@/store/actions/contentActions";
...
export default function AddBannerPage() {
  ...
  return (
    <div>
      <Breadcrumb />
      <PageHeader title="Add New Banner" description="Create a new promotional banner for the platform">
        <motion.button
      ...
      >
        <div className="flex items-center gap-3 mb-6 pb-5 border-b border-gray-100 dark:border-white/[0.06]">
          <div className="h-11 w-11 rounded-xl flex items-center justify-center" style={{ background: "rgba(15,105,176,0.1)" }}>
            <Image className="h-5 w-5 text-[#0F69B0]" />
          </div>
          <div>
            <h2 className="text-base font-black text-foreground">Banner Information</h2>
            <p className="text-xs text-muted-foreground font-medium mt-0.5">Fill in the details to create a new banner</p>
          </div>
        </div>
        <BannerForm initialData={{}} onSubmit={handleSubmit} onCancel={() => router.push("/content-ads/banners")} isLoading={isLoading} />
      </motion.div>
    </div>
  );
}