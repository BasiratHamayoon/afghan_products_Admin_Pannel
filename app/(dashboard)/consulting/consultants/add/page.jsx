...
import { createConsultant } from "@/store/actions/consultingActions";
...
export default function AddConsultantPage() {
  ...
  const handleSubmit = async (data) => {
    setIsLoading(true);
    const res = await dispatch(createConsultant(data));
    setIsLoading(false);
    if (res?.success) {
  ...
  return (
    <div>
      <Breadcrumb />
      <PageHeader title="Add New Consultant" description="Register a new consultant to the platform">
        <motion.button
      ...
      >
        <div className="flex items-center gap-3 mb-6 pb-5 border-b border-gray-100 dark:border-white/[0.06]">
          ...
          <div>
            <h2 className="text-base font-black text-foreground">Consultant Information</h2>
            <p className="text-xs text-muted-foreground font-medium mt-0.5">
              Fill in the details below to register a new consultant
            </p>
          </div>
        </div>
        ...
        <ConsultantForm
          ...
          onCancel={() => router.push("/consulting")}
          isLoading={isLoading}
        />
      </motion.div>
    </div>
