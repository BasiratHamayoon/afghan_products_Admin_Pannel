    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] overflow-hidden">
        <div className="flex border-b border-gray-100 dark:border-white/[0.06]">
          {[100, 80, 70, 65].map((w, i) => <div key={i} className="px-4 py-4"><Skeleton className="h-4 rounded" style={{ width: w }} /></div>)}
        </div>
        <div className="p-4 border-b border-gray-50 dark:border-white/[0.04]">
          <div className="flex gap-2"><Skeleton className="h-10 flex-1 rounded-xl" /><Skeleton className="h-10 w-24 rounded-xl" /><Skeleton className="h-10 w-20 rounded-xl" /></div>
        </div>
        <div className="p-4">{Array.from({ length: 8 }).map((_, i) => <RowSkeleton key={i} />)}</div>
      </div>
    </div>
  );
}