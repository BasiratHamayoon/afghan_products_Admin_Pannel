...
function ProductTypeDetailContent() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setItemData(null);
      try {
        const res = await dispatch(fetchProductTypeBySlug(slug));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)] flex flex-col items-center text-center">
          ...
          {itemData?.categoryName && (
            <p className="text-xs text-[#0F69B0] font-semibold mb-0.5">{itemData.categoryName}</p>
          )}
          {itemData?.subCategoryName && (
            <p className="text-xs text-purple-600 font-semibold mb-1">↳ {itemData.subCategoryName}</p>
          )}
