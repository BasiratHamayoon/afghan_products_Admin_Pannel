...
function DetailContent() {
  ...
  const TYPE_CONFIG = {
    category: {
      label: t("categories.categoryDetail"),
      labelShort: "Category",
      backPath: "/categories",
      ...
      emoji: "📦",
      archiveSuccess: t("categories.categoryArchived"),
      unarchiveSuccess: t("categories.categoryUnarchived"),
      deleteSuccess: t("categories.categoryDeleted"),
      information: t("categories.categoryInformation"),
    },
    subCategory: {
      label: t("categories.subcategoryDetail"),
      labelShort: "Subcategory",
      ...
      emoji: "📁",
      archiveSuccess: t("categories.subcategoryArchived"),
      unarchiveSuccess: t("categories.subcategoryUnarchived"),
      deleteSuccess: t("categories.subcategoryDeleted"),
      information: t("categories.subcategoryInformation"),
    },
    productType: {
      label: t("categories.detail"),
      labelShort: "Product Type",
      ...
      unarchiveSuccess: t("categories.categoryUnarchived"),
      deleteSuccess: t("categories.categoryDeleted"),
      information: t("categories.information"),
    },
  };
  ...
  useEffect(() => {
    if (!id || !fetchKey || STATIC_SEGMENTS.includes(id)) { setIsFetching(false); return; }
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!fetchFn) {
          if (isMountedRef.current) { setNotFound(true); setIsFetching(false); }
        ...
        if (isMountedRef.current) { delete _fetchCache[fetchKey]; setNotFound(true); }
      } finally {
  ...
  const handleArchiveConfirm = async () => {
    ...
    setIsActioning(true);
    try {
      const fn = archiveDialog.action === "archive" ? config.archiveFn : config.unarchiveFn;
      const res = await dispatch(fn(itemData.id));
      if (res?.success) {
        toast.success(archiveDialog.action === "archive" ? config.archiveSuccess : config.unarchiveSuccess);
    ...
    return (
      <div className="flex items-center justify-center py-32">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground font-medium">{t("categories.loadingCategory")}</p>
        </div>
      </div>
    );
  }
  ...
  if (notFound || !itemData) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4">
        ...
        <h2 className="text-lg font-black text-foreground">{config.labelShort} {t("categories.itemNotFound")}</h2>
  ...
  if (typeParam === "category" && itemData?.subCategoryCount !== undefined) {
    detailFields.splice(4, 0, { label: t("categories.subcategoriesCount"), value: String(itemData.subCategoryCount), icon: FolderTree });
  }

  if (typeParam === "subCategory" && itemData?.categoryName) {
    detailFields.splice(1, 0, { label: t("categories.parentCategory"), value: itemData.categoryName, icon: FolderOpen });
  }

  if (typeParam === "productType") {
    if (itemData?.categoryName) detailFields.splice(1, 0, { label: t("categories.categoryField"), value: itemData.categoryName, icon: FolderOpen });
    if (itemData?.subCategoryName) detailFields.splice(2, 0, { label: t("categories.subcategoryField"), value: itemData.subCategoryName, icon: Layers });
  }

  return (
    <div className="space-y-5">
      <Breadcrumb />
      <PageHeader title={config.label} description={itemData?.name || ""}>
        <div className="flex items-center gap-2">
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => router.push(config.backPath)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/[0.08] text-sm font-bold text-muted-foreground hover:bg-gray-50 dark:hover:bg-white/[0.04] transition-colors cursor-pointer">
            <ArrowLeft className="h-4 w-4 rtl-mirror" />{t("categories.back")}
          </motion.button>
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => router.push(`/categories/add?type=${typeParam}&edit=${id}`)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-white cursor-pointer shadow-lg shadow-[#0F69B0]/25" style={{ background: "linear-gradient(135deg, #0F69B0 0%, #0c5a9e 100%)" }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)] flex flex-col items-center text-center">
          ...
          <div className="flex items-center gap-2 mt-5 pt-4 border-t border-gray-100 dark:border-white/[0.06] w-full justify-center flex-wrap">
            <button onClick={() => router.push(`/categories/add?type=${typeParam}&edit=${id}`)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-[#0F69B0] hover:bg-[#0F69B0]/[0.08] transition-colors cursor-pointer border border-[#0F69B0]/20">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="lg:col-span-2 rounded-2xl p-6 bg-white dark:bg-[#0f1420] border border-gray-100 dark:border-white/[0.06] shadow-[0_2px_12px_rgba(15,105,176,0.06)]">
          <h3 className="text-sm font-black text-foreground mb-5">{config.information}</h3>
      ...
      <ConfirmDialog
        ...
        description={`${t("categories.deleteCategoryDesc")} "${itemData?.name}"${t("categories.deleteSuffix")}`}
