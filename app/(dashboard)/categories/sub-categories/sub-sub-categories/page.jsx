...
import { Plus, FolderTree, FolderOpen, Package, Layers } from "lucide-react";
...
export default function SubSubCategoriesPage() {
  ...
  useEffect(() => {
    setTimeout(() => {
      const level3 = dummySubCategories.filter((s) => s.level === 3);
      setData(level3);
      setIsLoading(false);
    }, 500);
  }, []);
  ...
  const totalProducts = data.reduce((acc, s) => acc + (s.productsCount || 0), 0);
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        description="Manage level 3 nested categories under subcategories"
      >
      ...
      >
        ...
        <div className="p-5">
          {isLoading ? (
            <LoadingSpinner size="lg" text="Loading sub-sub categories..." className="py-16" />
          ) : filteredData.length === 0 ? (
            <EmptyState
              icon={Layers}
              title="No sub-sub categories found"
              description="Level 3 categories appear here once created under subcategories"
              action={
                <button
                  onClick={() => router.push("/categories/add")}
