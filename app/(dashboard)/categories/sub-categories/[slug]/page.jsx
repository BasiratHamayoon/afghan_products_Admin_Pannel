...
import {
  ...
  Loader2, XCircle, Calendar, Hash, FolderOpen, Layers, Package,
} from "lucide-react";
...
function SubCategoryDetailContent() {
  ...
  const handleArchiveConfirm = async () => {
    ...
    try {
      ...
      if (res?.success) {
        toast.success(archiveDialog.action === "archive" ? "Subcategory archived" : "Subcategory unarchived");
    ...
    return (
      <div className="flex items-center justify-center py-32">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground font-medium">Loading subcategory...</p>
        </div>
      </div>
    );
  }
  ...
  if (notFound || !itemData) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4">
        ...
        <h2 className="text-lg font-black text-foreground">Subcategory not found</h2>
  ...
  const detailFields = [
    ...
    { label: "Parent Category", value: itemData?.categoryName || "—", icon: FolderOpen },
  ...
  return (
    <div className="space-y-5">
      <Breadcrumb />
      <PageHeader title="Subcategory Detail" description={itemData?.name || ""}>
        <div className="flex items-center gap-2 flex-wrap justify-end">
          ...
          <motion.button
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.97 }}
            onClick={() => router.push(`/categories/sub-categories/add?edit=${itemData.id}&slug=${itemData.slug || ""}`)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        >
          <h3 className="text-sm font-black text-foreground mb-5">Subcategory Information</h3>
